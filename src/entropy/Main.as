package entropy {
	...
	public class Main extends Sprite {
		...
		public function Main() {
			...
			if (stage) initMap(); //initTitle();
			else addEventListener(Event.ADDED_TO_STAGE, initMap);
			//
		}
		
		private function initTitle(e:Event = null):void
		{
			removeEventListener(Event.ADDED_TO_STAGE, initTitle);
			ref = new FileReference();
			title = new titleScreen(ref);
			addChild(title);
			addEventListener(Event.REMOVED_FROM_STAGE, startGame);
			
		}
		
		private function startGame(e:Event):void
		{
			if (e.target != title)//if we are not removing the title
				return;
			else
			{
				removeEventListener(Event.REMOVED_FROM_STAGE, startGame);
				refS = title.inString;
				trace(refS);
				//delete title;
				ref = new FileReference();
		...
		private function initMap(e:Event = null):void {
			//also autogenerated; the "entry point" is directly after this line
			removeEventListener(Event.ADDED_TO_STAGE, initMap);
